...
import model.LottoPossibles;

public class LottoPossiblesService {
	public void addCombination(ArrayList<Integer> list)
	{
		String query = "INSERT INTO " + LottoPossibles.TABLE + " VALUES(?,?,?,?,?,?,?)";
	...
	{
		String query = "SELECT COUNT(*) AS TOTAL FROM " + LottoPossibles.TABLE;
		int count = 0;
		try {
			PreparedStatement ps = LottoDB.getConnection().prepareStatement(query);
	...
	{
		String query = "SELECT * FROM " + LottoPossibles.TABLE + " WHERE "
				+ LottoPossibles.COL_NUMBER1 + " = ? AND "
				+ LottoPossibles.COL_NUMBER2 + " = ? AND "
				+ LottoPossibles.COL_NUMBER3 + " = ? AND "
				+ LottoPossibles.COL_NUMBER4 + " = ? AND "
				+ LottoPossibles.COL_NUMBER5 + " = ? AND "
				+ LottoPossibles.COL_NUMBER6 + " = ?";
		boolean exist = false;
		try {
